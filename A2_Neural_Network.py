...
def NN(set1X_train, set1X_test, set1y_train, set1y_test,set2X_train, set2X_test, set2y_train, set2y_test):
    print("ANN")
    ...
    # Extracting the score
    print(nn_classifier_scaled2.score(set2X_train, set2y_train))
    # Testing accuracy on the test data
    nn_classifier_scaled1.score(set2X_test, set2y_test)
    print(nn_classifier_scaled2.score(set2X_test, set2y_test))
    ...
    plt = plot_loss_curve(rf_best1, title, set2X_train, set2y_train, axes=None, ylim=None, cv=None, n_jobs=None,
                              train_sizes=np.linspace(.1, 1.0, 5))
    plt.savefig('Data1 Neural Network LossCurve'+timestr+'.png')
    plt.show()

    # Building a 10 fold Cross Validated GridSearchCV object
    grid_object = GridSearchCV(estimator=nn_classifier_scaled2, param_grid=grid_params, scoring='accuracy', cv=8,
                               n_jobs=-1)
    ...
    plt = plot_loss_curve(rf_best2, title, set2X_train, set2y_train, axes=None, ylim=None, cv=None, n_jobs=None,
                              train_sizes=np.linspace(.1, 1.0, 5))
    plt.savefig('Data2 Neural Network LossCurve'+timestr+'.png')
    plt.show()
    ...
    vc.make_validation(set2X_train, set2y_train, 'Set2', rf_best2, "Neural Network", 'nn__alpha',
                       nn__alphas)
    ...
    plt.savefig('Data2 Neural Network Learning Curve' + timestr + '.png')
    plt.show()
